...
fun IosNotificationStackConfig(
    ...
    Column {
        // 整组沉底开关（独立于 feature 总开关）
        var sinkAll by remember {
            mutableStateOf(
                PrefsHelper.getFeatureBoolean(context, packageName, "sink_all", true)
            )
        }
        Row(
            modifier = Modifier
            ...
            Switch(
                ...
                onCheckedChange = {
                    sinkAll = it
                    PrefsHelper.setFeatureBoolean(context, packageName, "sink_all", it)
                },
